...
const movie_array = JSON.parse(localStorage.getItem("movieArray")) || [];
...
admin_form.addEventListener("submit", function (event) {
  ...
  const movie = new Movie(images, movie_name, release_date, imdb_rating);
  ...
  localStorage.setItem("movieArray", JSON.stringify(movie_array));
});
